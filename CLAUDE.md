...
매출 자격 판정 (**2026-06-15 기준 반전**): 공고 변경으로 **'직전년도(2025) 매출 2억원 미만 또는 최근 3개년 연평균 2억원 미만'이 지원 대상**이 됨. `calcRevenue()`는 `ok = (rev2025 < limit) || (avg < limit)` — 둘 중 하나라도 2억 미만이면 자격, **둘 다 2억 이상일 때만 제외**. ⚠️ 과거엔 정반대(`ok = avg >= limit`, 2억 이상이 자격)였으니 옛 코드·문서·드롭다운 문구를 그대로 믿지 말 것 (작업내역.md 5차 → 15차 변천 참조). 요약/드롭다운 문구와 `Code.gs` GPT 프롬프트 예시도 "매출 2억원 미만"으로 동기화돼 있음.
